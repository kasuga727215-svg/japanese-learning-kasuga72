...
GEMINI_BANK_WORD_MAX_BATCH = 10
GEMINI_BANK_WORD_TIMEOUT_RETRY_BATCH = 4
GEMINI_BANK_VERB_MAX_BATCH = 4
GEMINI_BANK_VERB_TIMEOUT_RETRY_BATCH = 2
GEMINI_BANK_GRAMMAR_MAX_BATCH = 2
GEMINI_BANK_GRAMMAR_TIMEOUT_RETRY_BATCH = 1
GEMINI_BANK_SNS_MAX_BATCH = 8
GEMINI_BANK_SNS_TIMEOUT_RETRY_BATCH = 4
...
_GEMINI_BANK_USAGE_BACKFILL_DONE = False
...
def gemini_daily_fresh_step_key(pack_type):
    return f"daily_fresh:{str(pack_type or '').strip().lower()}"
...
def gemini_step_key_from_payload(step):
    ...
    if stage == "daily_fresh":
        ...
        return gemini_daily_fresh_step_key(pack_type) if pack_type else ""
    if stage != "refill":
        return ""
...
def gemini_bank_not_ready_pools(job, recent_usage=None):
    ...
    for step in steps:
        ...
        fresh_stock = gemini_bank_count_fresh_stock(
            item_type,
            level,
            recent_used_keys=gemini_bank_recent_keys_for_item_type(recent_usage, item_type),
        )
...
def mark_gemini_generation_step_completed(job_id, item_type, level):
    return mark_gemini_generation_step_completed_key(job_id, gemini_refill_step_key(item_type, level))


def mark_gemini_generation_step_completed_key(job_id, key):
        ...
        if DATABASE_URL:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("ALTER TABLE gemini_item_bank ADD COLUMN IF NOT EXISTS daily_batch_id TEXT NOT NULL DEFAULT ''")
                    cur.execute("ALTER TABLE gemini_item_bank ADD COLUMN IF NOT EXISTS generated_for_date DATE")
                    cur.execute("ALTER TABLE gemini_item_bank ADD COLUMN IF NOT EXISTS generated_source TEXT NOT NULL DEFAULT ''")
                    cur.execute("CREATE INDEX IF NOT EXISTS idx_gemini_item_bank_daily_batch ON gemini_item_bank(daily_batch_id, item_type, jlpt_level)")
            ...
            with sqlite3.connect(SQLITE_SETTINGS_FILE, timeout=10) as conn:
                ...
                columns = {row[1] for row in conn.execute("PRAGMA table_info(gemini_item_bank)").fetchall()}
                if "daily_batch_id" not in columns:
                    conn.execute("ALTER TABLE gemini_item_bank ADD COLUMN daily_batch_id TEXT NOT NULL DEFAULT ''")
                if "generated_for_date" not in columns:
                    conn.execute("ALTER TABLE gemini_item_bank ADD COLUMN generated_for_date TEXT")
                if "generated_source" not in columns:
                    conn.execute("ALTER TABLE gemini_item_bank ADD COLUMN generated_source TEXT NOT NULL DEFAULT ''")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_gemini_item_bank_daily_batch ON gemini_item_bank(daily_batch_id, item_type, jlpt_level)")
...
def gemini_bank_material_key_candidates(item, item_type):
    if not isinstance(item, dict):
        return set()
    fields = (
        ("normalized_key", "word", "surface", "term", "vocab_word", "base_form")
        if item_type == "word"
        else ("normalized_key", "dictionary_form", "surface", "base_form", "word", "term", "base")
    )
    keys = set()
    for field in fields:
        value = item.get(field)
        if isinstance(value, dict):
            value = value.get("dictionary") or value.get("surface") or ""
        text = str(value or "").strip()
        if not text:
            continue
        candidates = [text]
        for delimiter in ("（", "(", " - ", "｜", "|"):
            if delimiter in text:
                candidates.append(text.split(delimiter, 1)[0])
        for candidate in candidates:
            key = normalize_vocab_key(candidate)
            if key:
                keys.add(key)
    forms = item.get("forms")
    if item_type == "verb" and isinstance(forms, dict):
        key = normalize_vocab_key(forms.get("dictionary") or "")
        if key:
            keys.add(key)
    return keys


def gemini_bank_extract_material_usage_keys(material):
    usage = {"word": set(), "verb": set()}
    if not isinstance(material, dict):
        return usage
    vocab_items = material.get("vocab") or material.get("vocabulary") or []
    if isinstance(vocab_items, list):
        for item in vocab_items:
            usage["word"].update(gemini_bank_material_key_candidates(item, "word"))
    verb_items = material.get("verbs") or []
    if isinstance(verb_items, list):
        for item in verb_items:
            usage["verb"].update(gemini_bank_material_key_candidates(item, "verb"))
    return usage


def gemini_bank_recent_material_rows(days=None, material_date=None, limit=600):
    day_count = max(0, int(days if days is not None else GEMINI_BANK_RECENT_EXCLUSION_DAYS))
    anchor = parse_material_date(material_date) or taipei_now().date()
    start = anchor - timedelta(days=day_count)
    rows = []
    try:
        if DATABASE_URL:
            ensure_database()
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        """
                        SELECT material_json, COALESCE(material_date::text, date, created_at::text)
                        FROM materials
            ...
            if df.empty or "material_json" not in df.columns:
                ...
                material_rows = df[df["material_json"].astype(str).str.strip() != ""].tail(int(limit))
                raw_rows = [
                    (row.get("material_json", ""), row.get("material_date") or row.get("date") or row.get("created_at"))
                    for _, row in material_rows.iterrows()
                ]
        for material_json, date_value in raw_rows:
            try:
                payload = json.loads(material_json or "{}")
            except (TypeError, json.JSONDecodeError):
                continue
            payload_date = parse_material_date(date_value)
            if not payload_date and isinstance(payload, dict):
                payload_date = parse_material_date(payload.get("material_date") or payload.get("date"))
            if not payload_date or payload_date < start or payload_date > anchor:
                continue
            rows.append({"material": payload, "material_date": payload_date.isoformat()})
    except Exception as exc:
        print(f"[gemini-bank] recent material usage read failed reason={exc}")
    return rows


def gemini_bank_material_usage_snapshot(days=None, material_date=None):
    snapshot = {
        "word": set(),
        "verb": set(),
        "dates": {"word": {}, "verb": {}},
    ...
    for row in gemini_bank_recent_material_rows(days=days, material_date=material_date):
        material_date_iso = row["material_date"]
        usage = gemini_bank_extract_material_usage_keys(row["material"])
        if usage["word"] or usage["verb"]:
            snapshot["material_count"] += 1
        for item_type in ("word", "verb"):
            for key in usage[item_type]:
                snapshot[item_type].add(key)
                previous = snapshot["dates"][item_type].get(key, "")
                if not previous or material_date_iso > previous:
                    snapshot["dates"][item_type][key] = material_date_iso
    return snapshot
...
def gemini_bank_recent_usage_keys(days=None, material_date=None):
    snapshot = gemini_bank_material_usage_snapshot(days=days, material_date=material_date)
    print(
        "[gemini-bank] recent_used_keys "
        f"word={len(snapshot['word'])} verb={len(snapshot['verb'])} "
        f"days={max(0, int(days if days is not None else GEMINI_BANK_RECENT_EXCLUSION_DAYS))}"
    )
    for item_type in ("word", "verb"):
        sample_keys = sorted(snapshot[item_type])
        logged_keys = set()
        if "申し込む" in snapshot[item_type]:
            print(f"[gemini-bank] exclude recent key item_type={item_type} key={log_safe_text('申し込む')}")
            logged_keys.add("申し込む")
        for key in sample_keys[:3]:
            if key in logged_keys:
                continue
            print(f"[gemini-bank] exclude recent key item_type={item_type} key={log_safe_text(key)}")
    return snapshot


def gemini_bank_usage_timestamp(material_date_iso):
    parsed = parse_material_date(material_date_iso)
    if not parsed:
        return utc_now_iso()
    return datetime.fromisoformat(f"{parsed.isoformat()}T00:00:00+00:00").isoformat(timespec="seconds")


def backfill_gemini_bank_usage_from_materials(days=None, material_date=None):
    global _GEMINI_BANK_USAGE_BACKFILL_DONE
    if _GEMINI_BANK_USAGE_BACKFILL_DONE:
        return {"updated": 0, "material_count": 0, "skipped": True}
    day_count = max(0, int(days if days is not None else GEMINI_BANK_USAGE_BACKFILL_DAYS))
    if day_count <= 0:
        _GEMINI_BANK_USAGE_BACKFILL_DONE = True
        return {"updated": 0, "material_count": 0, "skipped": True}
    ensure_gemini_item_bank_store()
    snapshot = gemini_bank_material_usage_snapshot(days=day_count, material_date=material_date or get_today_taipei_date())
    updated = 0
    now = utc_now_iso()
    try:
        if DATABASE_URL:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    for item_type in ("word", "verb"):
                        for key, used_date in snapshot["dates"][item_type].items():
                            used_at = gemini_bank_usage_timestamp(used_date)
                            cur.execute(
                                """
                                UPDATE gemini_item_bank
                                SET used_count = CASE WHEN COALESCE(used_count, 0) = 0 THEN 1 ELSE used_count END,
                                    first_used_at = COALESCE(first_used_at, %s),
                                    last_used_at = COALESCE(last_used_at, %s),
                                    updated_at = %s
                                WHERE item_type = %s AND normalized_key = %s
                                  AND (COALESCE(used_count, 0) = 0 OR last_used_at IS NULL)
                                """,
                                (used_at, used_at, now, item_type, key),
                            )
            ...
            with sqlite3.connect(SQLITE_SETTINGS_FILE, timeout=10) as conn:
                ...
                conn.commit()
        _GEMINI_BANK_USAGE_BACKFILL_DONE = True
        print(f"[gemini-bank] backfill usage from materials count={updated} material_count={snapshot['material_count']} days={day_count}")
        return {"updated": updated, "material_count": snapshot["material_count"], "skipped": False}
    except Exception as exc:
        print(f"[gemini-bank] backfill usage from materials failed reason={exc}")
        return {"updated": updated, "material_count": snapshot["material_count"], "skipped": False, "warning": str(exc)}


def gemini_bank_recent_keys_for_item_type(recent_usage, item_type):
    if GEMINI_BANK_ALLOW_RECENT_REUSE or item_type not in {"word", "verb"}:
        return set()
...
def expand_gemini_bank_light_payload(item_type, payload, fallback_level):
    ...
    if item_type == "word":
        ...
        normalized_level = normalize_gemini_bank_level(payload.get("l") or payload.get("jlpt_level") or payload.get("level") or fallback_level, "word")
        return {
            **payload,
            "word": word,
            ...
            "category": simple_text(payload.get("c") or payload.get("category")) or ("approved_slang" if normalized_level == "SNS" else "general"),
            ...
            "example_translation_zh": simple_text(payload.get("ex_zh") or payload.get("example_translation_zh") or payload.get("example_zh")),
        }
    if item_type == "verb":
        surface = simple_text(payload.get("d") or payload.get("surface") or payload.get("dictionary_form") or payload.get("base_form"))
        forms = {}
        if simple_text(payload.get("f_te")):
            forms["te_form"] = simple_text(payload.get("f_te"))
        if simple_text(payload.get("f_nai")):
...
def gemini_bank_count_fresh_stock(item_type, level, recent_used_keys=None):
    ensure_gemini_item_bank_store()
    level = normalize_gemini_bank_level(level, item_type)
    ...
    if DATABASE_URL:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                params = [item_type, level, list(GEMINI_BANK_ACTIVE_STATUSES)]
                if recent_keys:
    ...
    recent_params = []
    if recent_keys:
        recent_clause = f"AND normalized_key NOT IN ({','.join(['?'] * len(recent_keys))})"
        recent_params = list(recent_keys)
    row = sqlite_one(
        f"""
        SELECT COUNT(*) AS count
        FROM gemini_item_bank
        WHERE item_type = ?
          ...
          {recent_clause}
        """,
        (item_type, level, *GEMINI_BANK_ACTIVE_STATUSES, *recent_params),
    )
    return int(row["count"] or 0)
...
def build_gemini_bank_prompt(item_type, level, count, exclude_keys=None, strategy=None):
    if item_type == "word":
        schema = {"items": [{"w": "料理", "r": "りょうり", "m": "料理", "p": "名詞", "l": level}]}
    ...
    return (
        "Return a JSON object only with an items array. Do not use Markdown, comments, explanations, or extra fields. "
        ...
        f"Compact JSON schema: {json.dumps(schema, ensure_ascii=False)}"
    )


def gemini_bank_response_schema(item_type, level, count):
    item_properties = {
        "word": {
            "w": {"type": "STRING"},
            "r": {"type": "STRING"},
...
def gemini_bank_max_output_tokens(item_type, request_count, timeout_count=0):
    if item_type == "word":
        base = 900
    elif item_type == "verb":
        base = 700
...
def quota_with_small_overage(quota, max_total, add_backup=True):
    clean_quota = {level: max(0, int(count or 0)) for level, count in (quota or {}).items() if int(count or 0) > 0}
    requested = dict(clean_quota)
    if add_backup:
        for level in clean_quota:
            if sum(requested.values()) >= max_total:
                break
            requested[level] = requested.get(level, 0) + 1
    while sum(requested.values()) > max_total and requested:
        for level in reversed(list(requested.keys())):
            if requested[level] > clean_quota.get(level, 0):
                requested[level] -= 1
                break
        else:
            break
...
def gemini_daily_pack_steps(settings):
    ...
    word_quota_all = build_level_quota(target_levels, int(settings.get("vocab_count") or 0), item_type="word")
...
def gemini_daily_exclude_keys_by_level(item_type, levels, recent_keys=None, max_per_level=10):
    recent = sorted({normalize_vocab_key(key) for key in (recent_keys or set()) if normalize_vocab_key(key)})
    payload = {}
    for level in levels:
        existing = sorted(set(gemini_bank_existing_keys(item_type, level, limit=max_per_level)))
        combined = []
        for key in [*recent, *existing]:
            if key and key not in combined:
                combined.append(key)
            if len(combined) >= max_per_level:
                break
        payload[level] = combined
    return payload


def build_gemini_daily_fresh_prompt(pack_type, item_type, requested_by_level, exclude_keys_by_level):
    if item_type == "word":
        schema = {
            "items": [
                {
    ...
    return (
        ...
        f"Daily Fresh JIT pack={pack_type}. Generate exactly this quota_by_level: "
        f"{json.dumps(requested_by_level, ensure_ascii=False)}. Total items={total}. "
...
def daily_fresh_row_matches_candidate_fetch(row, item_type, level, excluded_keys):
    return daily_fresh_candidate_rejection_reason(row, item_type, level, excluded_keys) == ""


def daily_fresh_candidate_rejection_reason(row, item_type, level, excluded_keys):
    key = vocabulary_pool_candidate_key(row)
    surface = vocabulary_pool_candidate_surface(row)
    if vocabulary_pool_candidate_level(row, item_type) != level:
        return "level_mismatch"
    if not key or not surface:
        return "empty_key_or_surface"
    if key in excluded_keys:
        return "excluded_generated_or_recent"
    category = first_text(row, ["category"]).lower() or "general"
    source = first_text(row, ["source"]).lower()
    quality = first_text(row, ["quality"]).lower() or "normal"
    status = first_text(row, ["status"]).lower() or "active"
    ...
    if score is not None and score < VOCABULARY_POOL_MIN_COMMONNESS_SCORE:
        return "commonness"
    if item_type == "verb":
        audit = audit_verb_candidate(row, row=row, log=True)
        if not audit.get("accepted"):
            return f"non_verb:{audit.get('reason') or 'unknown'}"
        return ""
    if not is_daily_fresh_word_pool_row(row):
        return "non_word"
    return ""
...
def daily_fresh_existing_bank_keys(item_type, levels):
    ...
    if not levels:
        return set()
    freshness_clause_pg = ""
    freshness_clause_sqlite = ""
    if item_type in {"word", "verb"}:
        freshness_clause_pg = """
                      AND (
                        COALESCE(used_count, 0) > 0
                        OR last_used_at IS NOT NULL
                        OR LOWER(COALESCE(status, '')) IN ('used', 'rejected')
                      )
        """
        freshness_clause_sqlite = """
              AND (
...
def fetch_daily_fresh_candidate_rows(item_type, level, limit, excluded_keys):
    ...
    def scan_candidate_rows(fetch_id_batch, fetch_row):
        rows = []
        ...
        while len(rows) < limit:
            ...
            print(
                f"[vocabulary-pool] candidate_id_fetch_success item_type={item_type} "
            ...
            for candidate_id in candidate_ids:
                try:
                    candidate_row = fetch_row(candidate_id)
                    ...
                    log_candidate_fetch_error(item_type, level, "row_fetch", candidate_id, "", row_exc)
                    continue
        ...
        if DATABASE_URL:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    def fetch_id_batch(batch_size, offset):
                        cur.execute(id_sql, (batch_size, offset))
                        return [row[0] for row in cur.fetchall() if row and row[0] is not None]

                    def fetch_row(candidate_id):
                        cur.execute(row_sql, (candidate_id,))
                        raw_row = cur.fetchone()
                        if not raw_row:
                            return None
                        columns = [desc[0] for desc in cur.description]
                        return sanitize_vocabulary_pool_candidate_row(dict(zip(columns, raw_row)))

                    return scan_candidate_rows(fetch_id_batch, fetch_row)
        with sqlite3.connect(SQLITE_SETTINGS_FILE, timeout=10) as conn:
            ...
            def fetch_id_batch(batch_size, offset):
                return [
                    row[0]
                    for row in conn.execute(id_sql, (batch_size, offset)).fetchall()
                    if row and row[0] is not None
                ]

            def fetch_row(candidate_id):
                row = conn.execute(row_sql, (candidate_id,)).fetchone()
                return sanitize_vocabulary_pool_candidate_row(dict(row)) if row else None
...
def select_daily_fresh_candidates(item_type, requested_by_level, recent_keys, pack_type=""):
    ...
    def collect_candidates_for_level(level, needed):
        ...
        rows = fetch_daily_fresh_candidate_rows(item_type, level, needed, excluded_keys | seen)
        candidates = []
        for row in rows:
    ...
    for shortage in shortages:
        ...
        remaining = int(shortage.get("remaining") or 0)
        fallback_levels = [
            level
        ...
        for fallback_level in fallback_levels:
            ...
            remaining -= len(fallback_candidates)
            print(
                "[fresh-candidate] redistributed_shortage "
                f"item_type={item_type} from={source_level} to={fallback_level} "
...
def build_gemini_daily_enrich_prompt(pack_type, item_type, candidates_by_level):
    ...
    return (
        ...
        "Examples must be short: ex Japanese max 15 chars, ex_zh Chinese max 15 chars. "
        "Do not explain grammar, do not add notes, do not add object keys inside item arrays. "
...
def chunked_list(items, chunk_size):
    ...
    return [list(items[index : index + chunk_size]) for index in range(0, len(items or []), chunk_size)]


def daily_fresh_enrich_chunk_size(item_type):
    if item_type in {"verb", "grammar"}:
        return 1
    return 2


def daily_fresh_candidates_by_level(candidates):
    grouped = {}
    for candidate in candidates or []:
        level = simple_text(candidate.get("level"))
        if not level:
            continue
        grouped.setdefault(level, []).append(candidate)
    return grouped
...
def cached_daily_fresh_candidate_plan(pack_state):
    if not isinstance(pack_state, dict):
        return None
    plan = pack_state.get("candidate_plan")
...
def clipped_compact_text(value, max_chars=15):
    return simple_text(value)[:max(0, int(max_chars or 0))]


def compact_gemini_bank_item_from_array(item_type, raw, fallback_level):
    if not isinstance(raw, list):
        return raw
    values = [simple_text(value) for value in raw]
    if item_type == "verb":
        if len(values) < 10:
            return {}
        return {
            ...
            "f_nai": values[6],
            "f_ta": values[7],
            "ex": clipped_compact_text(values[8], 15),
...
def gemini_bank_light_refill_count(item_type, needed, level=None, timeout_count=0):
    needed = max(1, int(needed or 1))
    timeout_count = max(0, int(timeout_count or 0))
    normalized_level = normalize_gemini_bank_level(level or "", item_type) if level else ""
    if timeout_count > 0:
        if item_type == "word" and normalized_level == "SNS":
            return GEMINI_BANK_SNS_TIMEOUT_RETRY_BATCH
        if item_type == "word":
            return GEMINI_BANK_WORD_TIMEOUT_RETRY_BATCH
        if item_type == "verb":
            return GEMINI_BANK_VERB_TIMEOUT_RETRY_BATCH
        if item_type == "grammar":
            return GEMINI_BANK_GRAMMAR_TIMEOUT_RETRY_BATCH
    if item_type == "word":
        if normalized_level == "SNS":
            return min(max(needed * 2, 5), GEMINI_BANK_SNS_MAX_BATCH)
        return min(max(needed * 2, 4), GEMINI_BANK_WORD_MAX_BATCH)
    if item_type == "verb":
        return min(max(needed * 2, 3), GEMINI_BANK_VERB_MAX_BATCH)
    if item_type == "grammar":
        return min(max(needed, 1), GEMINI_BANK_GRAMMAR_MAX_BATCH)
    return needed
...
def ensure_gemini_bank_level_capacity_for_generation(
    ...
    duplicate_streak=0,
    timeout_count=0,
    recent_used_keys=None,
):
    ensure_gemini_item_bank_store()
    level = normalize_gemini_bank_level(level, item_type)
    if target_stock is not None:
        target = max(0, int(target_stock or 0))
        before = gemini_bank_count_fresh_stock(item_type, level, recent_used_keys=recent_used_keys)
        active_total = gemini_bank_count_active_total(item_type, level)
    ...
    attempt_count = max(0, int(attempt_count or 0))
    timeout_count = max(0, int(timeout_count or 0))
    if target_stock is not None and attempt_count >= 2:
        print(
            f"[gemini-bank] stop refill item_type={item_type} level={level} "
            f"reason=max_calls_reached attempts={attempt_count} fresh_stock={before} target_fresh={target}"
        )
        return {
            "active_total": active_total if target_stock is not None else None,
            "fresh_stock": before,
            "target_fresh_stock": target,
            "missing_count": max(0, int((target or 0) - before)),
            "requested": 0,
            "inserted": 0,
            "skipped": 0,
            "fresh_stock_after": before,
            ...
            "continue_same_step": False,
            "skipped_refill": True,
            "warning": "gemini_bank_refill_stopped_after_retries",
    ...
    if target_stock is not None:
        request_count = gemini_bank_light_refill_count(item_type, needed, level=level, timeout_count=timeout_count)
    else:
    ...
    try:
        exclude_limit = 10 if timeout_count > 0 else GEMINI_BANK_MAX_EXCLUDE_KEYS
        exclude_keys = sorted(set(gemini_bank_existing_keys(item_type, level, limit=exclude_limit)))[:exclude_limit]
        ...
        max_output_tokens = gemini_bank_max_output_tokens(item_type, request_count, timeout_count=timeout_count)
        raw_text = call_gemini(
            prompt,
            timeout_seconds=GEMINI_BANK_STAGE_TIMEOUT_SECONDS,
            response_mime_type="application/json",
            response_schema=gemini_bank_response_schema(item_type, level, request_count),
...
def gemini_bank_stage_plan(settings):
    return [
        step.get("pack_type") or step.get("stage")
        for step in gemini_daily_pack_steps(settings)
        if isinstance(step, dict)
    ]


def gemini_bank_step_plan(settings, recent_usage=None):
    steps = gemini_daily_pack_steps(settings)
    target_levels = settings_target_levels(settings)
...
def select_gemini_bank_items(item_type, quota, recent_used_keys=None):
    ensure_gemini_item_bank_store()
    selected = []
    now = utc_now_iso()
    ...
    for level, count in quota.items():
        ...
        if DATABASE_URL:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    recent_clause = "AND NOT (normalized_key = ANY(%s))" if recent_keys else ""
                    params = [item_type, list(GEMINI_BANK_ACTIVE_STATUSES), query_level]
                    if recent_keys:
                        params.append(list(recent_keys))
            ...
            with sqlite3.connect(SQLITE_SETTINGS_FILE, timeout=10) as conn:
                ...
                rows = conn.execute(
                    ...
                    LIMIT ?
                    """,
                    (item_type, *GEMINI_BANK_ACTIVE_STATUSES, query_level, *recent_params, int(count)),
                ).fetchall()
        ...
        recent_hit_count = len({row.get("normalized_key") if isinstance(row, dict) else row[2] for row in selected if (row.get("item_type") if isinstance(row, dict) else item_type) == item_type} & recent_keys)
        print(f"[gemini-bank] select item_type={item_type} level={level} selected={len(rows)} excluded_recent={len(recent_keys)} recent_hit={recent_hit_count}")
    return selected


def select_gemini_bank_items_by_batch(item_type, quota, daily_batch_id, recent_used_keys=None):
    ensure_gemini_item_bank_store()
    selected = []
    now = utc_now_iso()
    daily_batch_id = simple_text(daily_batch_id)
...
def select_gemini_bank_top_up_items(item_type, deficit, excluded_keys=None, recent_used_keys=None, exclude_daily_batch_id=""):
    ...
    if limit <= 0:
        return []
    excluded = {normalize_vocab_key(key) for key in (excluded_keys or set()) if normalize_vocab_key(key)}
    excluded.update(gemini_bank_recent_keys_for_item_type({item_type: recent_used_keys or set()}, item_type))
    exclude_daily_batch_id = simple_text(exclude_daily_batch_id)
    now = utc_now_iso()
    fresh_clause_pg = "" if GEMINI_BANK_ALLOW_RECENT_REUSE else "AND (COALESCE(used_count, 0) = 0 OR last_used_at IS NULL)"
    fresh_clause_sqlite = fresh_clause_pg
    if DATABASE_URL:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                batch_clause = "AND COALESCE(daily_batch_id, '') <> %s" if exclude_daily_batch_id else ""
                params = [item_type, list(GEMINI_BANK_ACTIVE_STATUSES), LEVELS]
                if excluded:
                    params.append(list(excluded))
                if exclude_daily_batch_id:
                    params.append(exclude_daily_batch_id)
                params.append(limit)
                cur.execute(
                    f"""
                    SELECT id, item_type, normalized_key, display_text, reading, jlpt_level, category, source, payload_json
        ...
        with sqlite3.connect(SQLITE_SETTINGS_FILE, timeout=10) as conn:
            ...
            batch_clause = ""
            if exclude_daily_batch_id:
                batch_clause = "AND COALESCE(daily_batch_id, '') <> ?"
                params.append(exclude_daily_batch_id)
            params.append(limit)
            rows = conn.execute(
                f"""
...
def generate_daily_material_from_gemini_bank(posted_settings=None, app_url=None):
    ...
    try:
        ...
        recent_usage = gemini_bank_recent_usage_keys(
            days=GEMINI_BANK_RECENT_EXCLUSION_DAYS,
            material_date=get_today_taipei_date(),
        )
        recent_word_keys = gemini_bank_recent_keys_for_item_type(recent_usage, "word")
        recent_verb_keys = gemini_bank_recent_keys_for_item_type(recent_usage, "verb")
        bank_refill = {
        ...
        selected_words = select_gemini_bank_items("word", word_quota, recent_used_keys=recent_word_keys)
        selected_verbs = select_gemini_bank_items("verb", verb_quota, recent_used_keys=recent_verb_keys)
            ...
            material["metadata"].update(
                {
                    ...
                    "recent_exclusion_days": GEMINI_BANK_RECENT_EXCLUSION_DAYS,
                    "recent_used_counts": {"word": len(recent_word_keys), "verb": len(recent_verb_keys)},
                    "usage_backfill": backfill_stats,
                    "resolved_settings": {
                        "generation_trigger": "manual",
                        "settings_source": settings_source,
...
def run_gemini_daily_fresh_pack(job_id, pack_type):
    job = load_gemini_generation_job(job_id)
    if not job:
        return {"ok": False, "error": "gemini_job_not_found", "reason": "job_id not found"}, 404
    pack_type = str(pack_type or "").strip().lower()
    steps = gemini_job_json(job.get("planned_steps_json"), [])
    step = next(
        (
            item
            for item in steps
            if isinstance(item, dict)
            and str(item.get("stage") or "").strip().lower() == "daily_fresh"
            and str(item.get("pack_type") or "").strip().lower() == pack_type
        ),
        None,
    )
    if not step:
        return {"ok": False, "error": "gemini_unplanned_step_rejected", "reason": "daily fresh pack is not planned", "pack_type": pack_type}, 200
    if not GEMINI_API_KEY:
        return {
            "ok": False,
            "error": "gemini_generation_not_available",
    ...
    if item_type in {"word", "verb", "grammar"}:
        ...
        inserted_total = int(pack_state.get("inserted") or 0)
        skipped_total = int(pack_state.get("skipped") or 0)

        def persist_daily_fresh_pack_state(current_stage):
            pack_cache[pack_type] = make_json_safe(pack_state)
            cache["daily_fresh"] = pack_cache
            update_gemini_generation_job(
                job_id,
        ...
        def enrich_candidate_chunk(chunk_candidates, chunk_label):
            ...
            print(
                ...
                f"parsed={chunk_parse_stats.get('parsed', 0)}"
            )
            print(
                f"[gemini-bank] daily_fresh normalized pack={pack_type} chunk={chunk_label} "
            ...
            print(
                f"[daily-fresh-candidates] enrich_validated pack={pack_type} chunk={chunk_label} "
        ...
        try:
            ...
            if reason in {"json_parse_error", "json_truncated", "output_token_limit", "output_token_limit_config_error", "thinking_token_limit"}:
                if reason == "output_token_limit_config_error":
                    ...
                    message = "Gemini 補全資料時回傳不完整。已保留成功片段，系統將改用更小批次續跑。" if reason == "json_truncated" else "Gemini 未回傳合法 JSON，請稍後重試。"
                update_gemini_generation_job(
                    job_id,
                    status="failed",
                    current_stage=f"daily_fresh:{pack_type}:chunk_{chunk_index}",
    ...
    for attempt_index in range(max_attempts):
        ...
        try:
            ...
            if reason == "timeout" and not retry and not best_effort:
                continue
            if reason in {"json_parse_error", "json_truncated", "output_token_limit"} and not retry and not best_effort:
            ...
            if best_effort:
                ...
                break
            print(f"[gemini-bank] daily_fresh warning pack={pack_type} reason={reason}")
            break

    pack_cache[pack_type] = {
        "item_type": item_type,
        "requested_by_level": requested_by_level,
        "requested": requested_total,
        "inserted": inserted_total,
...
def run_gemini_generation_stage(job_id, stage, item_type=None, level=None, requested_count=None):
    ...
    timeout_count = int(previous_refill.get("timeout_count") or 0)
    target_stock = int(gemini_bank_target_stock(item_type, level, settings=settings))
    backfill_gemini_bank_usage_from_materials(material_date=job.get("material_date") or get_today_taipei_date())
    recent_usage = gemini_bank_recent_usage_keys(
        days=GEMINI_BANK_RECENT_EXCLUSION_DAYS,
    ...
    try:
        ...
        return {
            ...
            "timeout_count": refill.get("timeout_count"),
            "requested": refill.get("requested"),
            "completed_steps": completed_steps,
            "elapsed_ms": elapsed_ms,
        }, 200
    except Exception as exc:
        ...
        next_timeout_count = timeout_count + 1 if reason == "timeout" else timeout_count
        stop_pool = item_type == "grammar" or (reason == "timeout" and next_timeout_count >= 2)
        warning_entry = {
            ...
            "reason": reason,
            "message": str(exc)[:240],
            "continued": not stop_pool,
            "stopped_pool": stop_pool,
        }
        warnings.append(warning_entry)
        cache["warnings"] = warnings
        refill_cache = cache.get("refill") if isinstance(cache.get("refill"), dict) else {}
        current_stock = gemini_bank_count_fresh_stock(item_type, level, recent_used_keys=recent_keys)
        active_total = gemini_bank_count_active_total(item_type, level)
        failed_requested = gemini_bank_light_refill_count(
            item_type,
            max(1, target_stock - current_stock),
            level=level,
            timeout_count=timeout_count,
        )
        refill_cache[level] = {
            ...
            "message": str(exc)[:240],
            "continued": True,
            "requested": failed_requested,
            "fresh_stock": current_stock,
            "target_fresh_stock": target_stock,
            "active_total": active_total,
            "missing_count": max(0, target_stock - current_stock),
            "pool_ready": stop_pool,
            "continue_same_step": not stop_pool,
        ...
        if stop_pool:
            ...
            if item_type == "grammar":
                ...
                print(f"[gemini-bank] refill stopped item_type={item_type} level={level} reason={reason} timeout_count={next_timeout_count}")
            print(f"[gemini-step-runner] completed stage=refill item_type={item_type} level={level} warning={warning_code}")
        else:
            update_gemini_generation_job(
                job_id,
                status="running",
...
def finalize_gemini_generation_job(job_id, app_url=None):
    ...
    try:
        ...
        missing_required_steps = [
            ...
            if not step.startswith("grammar:") and step != "daily_fresh:grammar"
        ]
        missing_grammar_steps = [step for step in missing_steps if step.startswith("grammar:") or step == "daily_fresh:grammar"]
        ...
        settings = gemini_job_settings(job)
        backfill_stats = backfill_gemini_bank_usage_from_materials(material_date=job.get("material_date") or get_today_taipei_date())
        recent_usage = gemini_bank_recent_usage_keys(
            days=GEMINI_BANK_RECENT_EXCLUSION_DAYS,
            material_date=job.get("material_date") or get_today_taipei_date(),
        )
        ...
        print(
            "[gemini-finalize] "
            f"selected duplicate_recent={str(duplicate_recent).lower()} "
            f"recent_exclusion_days={GEMINI_BANK_RECENT_EXCLUSION_DAYS}"
        )
        ...
        bank_refill = {
            ...
            "verb": verbs_stage.get("daily_fresh", {}) if isinstance(verbs_stage, dict) else {},
            "grammar": grammar_stage.get("daily_fresh", {}) if isinstance(grammar_stage, dict) else {},
        }
        bank_warnings = []
        ...
        material.setdefault("metadata", {}).update(
            {
                ...
                "fallback_count": {
                    ...
                    "verb": top_up["verb"].get("added", 0),
                    "grammar": top_up["grammar"].get("added", 0),
                },
                "fallback_reason": "daily_batch_shortfall" if any(top_up[item].get("added", 0) for item in ("word", "verb", "grammar")) else "",
        ...
        print(f"[gemini-finalize] daily_batch_selected word={daily_fresh_counts['word']} verb={daily_fresh_counts['verb']} grammar={daily_fresh_counts['grammar']}")
        print(f"[gemini-finalize] fallback_selected word={top_up['word']['added']} verb={top_up['verb']['added']} grammar={top_up['grammar']['added']}")
        ...
        error_code = (
            ...
            or str(exc).startswith("insufficient_")
            else "gemini_stage_failed"
        )
        message = f"{reason}:{str(exc)[:240]}"
